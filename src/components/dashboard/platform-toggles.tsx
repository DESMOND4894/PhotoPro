...
const platformConfig: {
  ...
  },
  {
    id: "instagram",
    label: "Instagram",
    activeStyle: { background: 'linear-gradient(to right, #9333ea, #ec4899)' },
    activeClass: "text-white shadow-sm",
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    ...
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
        <path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1v-3.5a6.37 6.37 0 00-.79-.05A6.34 6.34 0 003.15 15.2a6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.34-6.34V8.65a8.35 8.35 0 004.76 1.48v-3.45a4.85 4.85 0 01-1-.09z" />
      </svg>
    ),
  },
];
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
      {platformConfig.map((p) => {
        const isActive = enabled.includes(p.id);
        return (
          <button
            key={p.id}
            onClick={() => toggle(p.id)}
            ...
            style={{
              ...
              ...(isActive ? p.activeStyle : inactiveStyle),
            }}
            className={isActive ? p.activeClass : ""}
          >
            {p.icon}
            {p.label}
          </button>
